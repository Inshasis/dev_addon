	...
	def calculate_item_values(self):
		...
		if not self.discount_amount_applied:
			for item in self.doc.get("items"):
				...
				elif item.parenttype == "Sales Invoice" and item.parenttype == "Sales Order" and item.parenttype == "Quotation" and item.parenttype == "Delivery Note":
					if item.item_group == "Products":
						item.amount = flt(item.surcharge_amount + item.chargeable_amount + item.amount1 + item.amount2 + item.amount3 + item.amount4 , item.precision("amount"))
				else:
